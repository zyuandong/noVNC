...
const UI = {
  ...
  initSettings() {
    ...
    qualityDropdown.appendChild(
      Object.assign(document.createElement("option"), { value: 0, label: "Static" })
    );
    qualityDropdown.appendChild(
      Object.assign(document.createElement("option"), { value: 1, label: "Low" })
    );
    qualityDropdown.appendChild(
      Object.assign(document.createElement("option"), { value: 2, label: "Medium" })
    );
    qualityDropdown.appendChild(
      Object.assign(document.createElement("option"), { value: 3, label: "High" })
    );
    qualityDropdown.appendChild(
      Object.assign(document.createElement("option"), { value: 4, label: "Extreme" })
    );
    if (supportsSharedArrayBuffers) {
      qualityDropdown.appendChild(
        Object.assign(document.createElement("option"), { value: 5, label: "Lossless" })
      );
    }
    qualityDropdown.appendChild(
      Object.assign(document.createElement("option"), { value: 10, label: "Custom" })
    );
  ...
  isControlPanelItemClick(e) {
    if (
      !(
        ...
        e.target.classList &&
        e.target.parentNode &&
        ((e.target.classList.contains("noVNC_button") &&
          e.target.parentNode.id !== "noVNC_modifiers") ||
  ...
  connect(event, password) {
    ...
    if (supportsBinaryClipboard()) {
      ...
      navigator.permissions.query({ name: "clipboard-read" }).then((result) => {
        Log.Debug("binary clipboard enabled");
      });
    }
    ...
    if (WebUtil.isInsideKasmVDI()) {
      ...
      UI._sessionTimeoutInterval = setInterval(function () {
        ...
        if (timeSinceLastActivityInS > idleDisconnectInS) {
          parent.postMessage(
            { action: "idle_session_timeout", value: "Idle session timeout exceeded" },
            "*"
          );
        } else {
          //send keep-alive
  ...
  applyResizeMode() {
    ...
    UI.rfb.scaleViewport = UI.getSetting("resize") === "scale";
    UI.rfb.resizeSession =
      UI.getSetting("resize") === "remote" ||
      (UI.rfb.forcedResolutionX && UI.rfb.forcedResolutionY);
    UI.rfb.idleDisconnect = UI.getSetting("idle_disconnect");
  ...
  updatePointerLockButton() {
    ...
    if (
      UI.connected &&
      (document.pointerLockElement !== undefined || document.mozPointerLockElement !== undefined)
    ) {
  ...
  toggleWebRTC() {
    if (UI.rfb) {
      if (typeof RTCPeerConnection === "undefined") {
        UI.showStatus(
          "This browser does not support WebRTC UDP Data Channels.",
          "warn",
          5000,
          true
        );
